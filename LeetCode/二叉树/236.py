...
class Solution1:
    def lowestCommonAncestor(self, root: 'TreeNode', p: 'TreeNode', q: 'TreeNode') -> 'TreeNode':
        ...
        if (root.val == p.val and self.is_exists(root, q)) or (root.val == q.val and self.is_exists(root, p)):
            return root
        elif self.is_exists(root.left, p) and self.is_exists(root.right, q) or (self.is_exists(root.left, q) and self.is_exists(root.right, p)):
            return root
        elif self.is_exists(root.left, p) and self.is_exists(root.left, q):
            return self.lowestCommonAncestor(root.left, p, q)
        elif self.is_exists(root.right, p) and self.is_exists(root.right, q):
